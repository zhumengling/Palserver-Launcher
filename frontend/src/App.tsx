...
function PlayerDialog({ player, onClose, onAction }: { player: main.Player; onClose: () => void; onAction: (r: main.ActionRequest) => void }) {
  const [kind, setKind] = useState('item'); const [value, setValue] = useState('Wood'); const [amount, setAmount] = useState(1); const [catalogOpen, setCatalogOpen] = useState(false);
  const send = (action: string, val = value, count = amount) => onAction(new main.ActionRequest({ action, userId: player.userId, value: val, amount: count }));
  return <><div className="modal-backdrop"><div className="modal wide"><div className="modal-header"><div><h2>{player.name}</h2><p>{player.userId}</p></div><button onClick={onClose}><X size={18}/></button></div>
    ...
    <div className="form-grid three"><label><span>给予类型</span><select value={kind} onChange={(e) => { const next = e.target.value; setKind(next); if (next === 'item') setValue('Wood'); if (next === 'pal') setValue('SheepBall'); }}><option value="item">道具</option><option value="pal">帕鲁</option><option value="exp">经验</option><option value="relic">捕获力</option><option value="tech">科技点</option><option value="bosstech">古代科技点</option></select></label><label><span>ID / 名称</span><div className="input-action"><input disabled={!['item','pal'].includes(kind)} value={value} onChange={(e) => setValue(e.target.value)}/>{['item','pal'].includes(kind) && <button type="button" title="打开目录" onClick={() => setCatalogOpen(true)}><Search size={15}/></button>}</div></label><label><span>数量</span><input type="number" min="1" value={amount} onChange={(e) => setAmount(Number(e.target.value))}/></label></div>
    ...
    {catalogOpen && <Suspense fallback={<div className="busy-layer"><RefreshCw className="spin" size={20}/><span>正在加载游戏数据...</span></div>}><GameCatalog kind={kind as 'item' | 'pal'} selected={value} onClose={() => setCatalogOpen(false)} onSelect={setValue}/></Suspense>}</>;
}
...
function MapView({ id }: { id: string }) { const [players, setPlayers] = useState<main.Player[]>([]); useEffect(() => { const load = () => API.GetPlayers(id).then(setPlayers).catch(() => setPlayers([])); load(); const timer = setInterval(load, 3000); return () => clearInterval(timer); }, [id]); return <section className="panel map-panel"><div className="panel-heading"><div><h2>在线地图</h2><p>根据 REST API 坐标实时定位</p></div><span className="badge success">{players.length} 在线</span></div><div className="map-canvas"><div className="map-grid"/>{players.map((p, index) => { const x = ((p.locationY - 157664.56) / 462.96 + 500) / 10; const y = ((p.locationX + 123467.16) / 462.96 + 500) / 10; return <div className="player-pin" title={`${p.name} (${p.locationX.toFixed(0)}, ${p.locationY.toFixed(0)})`} key={p.userId} style={{ left: `${Math.max(3, Math.min(97, x))}%`, top: `${Math.max(3, Math.min(97, y))}%` }}><span>{index + 1}</span><label>{p.name}</label></div>; })}{!players.length && <div className="map-empty"><Map size={28}/><span>没有可显示的在线玩家</span></div>}</div></section>; }
