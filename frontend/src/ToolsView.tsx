...
export default function ToolsView({ instance, running, run, onChanged }: { instance: main.ServerInstance; running: boolean; run: Run; onChanged: () => Promise<void> }) {
  ...
  const load = async () => {
    const [banList, officialBackups, serverPaths] = await Promise.all([API.ListBans(instance.id), API.ListOfficialBackups(instance.id), API.GetServerPaths(instance.id)]);
  ...
  return <div className="tools-grid">
    ...
    <section className="panel"><div className="panel-heading"><div><h2>常用目录</h2><p>旧版右键菜单中的全部目录入口</p></div><FolderOpen size={18}/></div><div className="directory-grid">{[['server','服务器目录'],['saved','Saved 目录'],['world','当前世界存档'],['config','配置目录'],['logs','PalDefender 日志'],['paldefender','PalDefender 配置']].map(([key,label]) => <button className="ghost" key={key} disabled={!paths[key]} onClick={() => key === 'paldefender' ? run('open-paldefender-config', () => API.OpenServerPath(instance.id, key), 'PalDefender 配置已打开') : run(`open-${key}`, () => API.OpenServerPath(instance.id, key))}><FolderOpen size={14}/>{label}<ExternalLink size={12}/></button>)}</div></section>
